            ...
            public void onClick(View view) {
                try {
                    ...
                    if (existingName.isEmpty() || newName.isEmpty() || pass.isEmpty()) {
                        Toast.makeText(UpdateName.this, "Заполните все поля.", Toast.LENGTH_SHORT).show();
                    } else {
                            ...
                            if (checkUsername) {
                                if (loggedInUserId == userId) {
                                    if (existingName.equals(newName)) {
                                        Toast.makeText(UpdateName.this, "Новое имя должно отличаться от старого.", Toast.LENGTH_SHORT).show();
                                    } else if (db.checkUsernameForUser(newName, userId)) { // Check if the new name already exists for the current user
                                        Toast.makeText(UpdateName.this, "Пользователь уже существует!", Toast.LENGTH_SHORT).show();
                                    } else {
                                        userAccount.setUsername(newName); // Update the username field with the new name
                                        db.updateName(userAccount);
                                        Toast.makeText(UpdateName.this, "Успешно", Toast.LENGTH_SHORT).show();
                                        Intent intent = new Intent(UpdateName.this, LogIn.class);
                                        startActivity(intent);
                                        finish();
                                    }
                                } else {
                                    Toast.makeText(UpdateName.this, "Вы не можете удалить другой аккаунт.", Toast.LENGTH_SHORT).show();
                                }
                            } else {
                                Toast.makeText(UpdateName.this, "Пользователь уже существует.", Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            Toast.makeText(UpdateName.this, "Неверное имя пользователя или пароль.", Toast.LENGTH_SHORT).show();
                        }
                    }
                } catch (Exception e) {
                    e.printStackTrace();
