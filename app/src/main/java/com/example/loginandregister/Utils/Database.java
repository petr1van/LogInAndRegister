...
public class Database extends SQLiteOpenHelper {
    ...
    public int getUserId(int id) {
        ...
        if (cursor.moveToFirst()) {
            do {
                id = cursor.getInt(cursor.getColumnIndex(Database.COLUMN_ID));
                String username = cursor.getString(cursor.getColumnIndex(Database.COLUMN_USERNAME));
                String password = cursor.getString(cursor.getColumnIndex(Database.COLUMN_PASSWORD));
...
// Закройте курсор и базу данных после использования
        cursor.close();
        db.close();
        return userId;
    }
